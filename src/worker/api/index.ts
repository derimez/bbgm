...
// ── Cross-device sync (custom fork) ─────────────────────────────────────────
// syncId is a stable per-league identity stored in gameAttributes so the same
// league shares one key across every device, regardless of its local lid.

// UUID v4 that works in NON-secure contexts (HTTP over the Tailscale hostname),
// where crypto.randomUUID() is undefined. getRandomValues IS available there.
...
// Read sync identity + display fields for the loaded league. When mint is true
// and the league has no syncId yet, generate and persist one (used by the
// manual "Sync ⬆" button on first push).
const bbgmSyncInfo = async (mint?: boolean) => {
	const row = await idb.cache.gameAttributes.get("syncId" as any);
	let syncId = (row?.value as string | undefined) ?? undefined;

	if (!syncId && mint) {
		syncId = genUUID();
		await league.setGameAttributes({ syncId } as any);
	}

	const userTid = g.get("userTid");
	const teamInfo = g.get("teamInfoCache")?.[userTid];
...
// Force a known syncId onto the loaded league (re-stamped after an auto-pull
// import so identity survives regardless of import keptKeys filtering).
const bbgmSyncStamp = async (syncId: string) => {
	await league.setGameAttributes({ syncId } as any);
};
